...
export default function Projects() {
  ...
  // Always fetch desktop data, conditional rendering handles mobile
  const {
    data: desktopData,
    isLoading: desktopLoading,
    isError: desktopError,
  } = useProjects(PAGE_SIZE, offset);
  ...
  const handlePreviousPage = () => updatePage(Math.max(1, currentPage - 1));

  const handleNextPage = () =>
    updatePage(hasMore ? currentPage + 1 : currentPage);

  const handlePageClick = (page: number) => updatePage(page);
